<x-admin-layout>
    <x-slot name="header">
        ...
        <nav aria-label="Breadcrumb" class="mt-2">
            <ol class="flex items-center gap-1 text-sm text-gray-600">
                ...
                <li>
                    <a href="{{ route('keluarga.create') }}" class="block transition hover:text-gray-700"> Tambah Data Keluarga </a>
                </li>
            </ol>
        </nav>
    </x-slot>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const kkError = document.getElementById("no_kk-error");
            kkInput.addEventListener("input", function() {
                const inputValue = kkInput.value;
                const apiUrl = "{{ route('checkNoKK') }}";

                fetch(`${apiUrl}?no_kk=${inputValue}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.exists) {
                            kkError.textContent = "Nomor KK yang dimasukan sudah ada dalam database.";
                        } else {
                            kkError.textContent = "";
                        }
                    });
