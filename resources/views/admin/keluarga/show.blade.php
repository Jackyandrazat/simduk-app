<x-admin-layout>
    <x-slot name="header">
        ...
        <nav aria-label="Breadcrumb" class="mt-2">
            <ol class="flex items-center gap-1 text-sm text-gray-600">
                ...
                <li>
                    <a href="{{ route('keluarga.show', ['keluarga' => $keluarga->id]) }}"
                        class="block transition hover:text-gray-700"> Detail Keluarga </a>
                </li>
            </ol>
        </nav>
    </x-slot>
    <div class="relative inline-block text-right py-2">
        {{-- <button id="toggleDropdown" type="button"
            class="float-right flex items-center justify-end px-4 py-2 mr-5 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-green-600 border border-transparent rounded-lg active:bg-green-600 hover:bg-green-900 focus:outline-none focus:shadow-outline-green">
    ...
    <div class="flow-root container px-6 mb-7">
        <div class="w-full bg-white shadow-md rounded-lg overflow-hidden">
            <img class="w-full h-32 object-cover" src="{{ url('windmill') }}/public/assets/img/bg-keluarga.png"
                alt="Keluarga">
            <div class="p-4">
                ...
                <div class="mt-4">
                    <table class="w-full mt-2 text-center">
                        ...
                        <tbody>
                            ...
                            @if (isset($istri))
                                <tr>
                                    <td class="p-2 font-semibold">{{ $istri->nama }}</td>
                                    <td class="p-2 text-gray-600">{{ $istri->nik }}</td>
                                    <td class="p-2 text-gray-600">{{ $istri->jenis_kelamin }}</td>
                                    <td class="p-2 text-gray-600">{{ $istri->tanggal_lahir }}</td>
                                    <td class="p-2 text-gray-600">{{ $istri->status }}</td>
                                    <td class="p-2 text-gray-600">{{ $istri->status_kk }}</td>
                                    <td class="p-2 text-gray-600">{{ $istri->pekerjaan }}</td>
                                </tr>
                            @endif
                            @if (isset($anak))
                                <tr>
                                    ...
                                    <td class="p-2 text-gray-600">{{ $anak->jenis_kelamin }}</td>
                                    <td class="p-2 text-gray-600">{{ $anak->tanggal_lahir }}</td>
                                    <td class="p-2 text-gray-600">{{ $anak->status }}</td>
